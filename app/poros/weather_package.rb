class WeatherPackage
  ...
  def initialize(current_weather, five_day_weather, eight_hour_weather, timezone_offset)
  ...
  def date_time_calculation(unix, offset = @offset)
    DateTime.strptime((unix - offset).to_s, '%s')
  end
  ...
  def current_forecast_calculation(weather_attributes)
    {
      ...
      temperature: weather_attributes[:temp],
  ...
  def eight_hour_forecast_calculation(weather_attributes)
    weather_attributes.map do |hour|
      {
